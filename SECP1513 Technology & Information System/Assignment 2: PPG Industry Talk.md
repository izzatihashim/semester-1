# Assignment 2: PPG Industry Talk
## 🏢 Details
**Date:** 20 November 2025 <br>
**Time:** 2:30pm - 3:30pm <br>
**Venue:** Hyflex classroom, N28A <br>
**Involvement:** All SECPH Students <br>
**Task:** Poster for a summary of the whole session <br>
### 🖼 Poster is placed at the bottom of the page! 

## 💡 Summary
- **Speaker 1: Introduction to PPG** <br>
-> Speaker: Ms. Ai Ling Ooi, APAC IT Lead <br>
-> Started off by giving an overview of the PPG Company, a renowned global company that is recognised around the world for their outstanding performance. I learnt about the efficient way in the PPG's work-life environment that enables them to achieve many significant milestones.

- **Speaker 2: Introduction to Global Infrastructure** <br>
-> Speaker: Vijaykumar Dhandapani, Cloud Architecture Lead <br>
-> Introduced PPG's Global Infrastructure on how it is organized and maintained. My key takeaway is the framework that they used to organize it, the concept of "Plan, Build, Run" taught me on how each theme and aspect come to work together into achieving one similar goal.

- **Speaker 3: Data Analytics** <br>
-> Speaker: Derrick Tan Horng Shi, Data Analytics Manager <br>
-> This session taught me on how essential it is to invest in a good data team as accessible & cleaned data is crucial in making data-driven business decision, improve overall efficiency and growth. I also learnt about the reality of working in the industry where data is much more bigger and messier than usual practice.

- **Speaker 4: System Analysis Program Development** <br>
-> Speaker: Patrick Thong, SAP Operations DTR Lead <br>
-> This session explains about the usage of SAP system in the PPG industry and how it helps to manage their operations especially in major areas such as Sales and Production.

## 📝 Reflections
This session has given me a clearer view on the PPG as a global company. Not only that, this session also let me understand about the roles and opportunities that I might get in the future as I dive deeper into the technology field. The explanation about global infrastructure, data analytics and SAP are also a very beneficial and great knowledge that I will values as a Computer Science student. I was left impressed with all the informations I have obtained about PPG that showcased how PPG is an undeniably great company to be collaborating with. Overall, I'm looking forward for more opportunities like this again in the future as it can widen my prospects and knowledge in the digital industry.

## 📌 Improvement Suggestions
- Increase the capacity of students that can attend so that students from other programs can also physically attend the session.

## 📸 Visual Highlights
![photo_2025-12-28_22-46-31](https://github.com/user-attachments/assets/5a05827d-600b-45bd-bc86-056a843d6ebb)
*One of the presented slide during the session*

![photo_2025-12-28_22-46-38](https://github.com/user-attachments/assets/a8d75632-3d1d-419c-9217-f986f679ebca)
*Group photo with everyone involved in the session*

## 🖼 Poster
![TIS G9 Poster_page-0001](https://github.com/user-attachments/assets/9123e5f9-dbc7-48f1-ba1d-27e5a86bb452)

## 🖇 Additional Links
🔗 Instruction & Rubric: <br>
🔗 Linkedin Post: <br>
